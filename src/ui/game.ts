...
export class Game {
  ...
  /** The manual's figures, drawn (see `ui/figure.ts`). Wrappers rather than
   *  call-site changes: the renderer moved out of this class on 2026-08-28 so
   *  a teaching CARD can draw one too, and `Game` keeps the three arguments
   *  it alone holds — the baked art, the facing, and the theme's colour names. */
  #figure(kind: FigureId): HTMLElement {
    return drawFigure(kind, {
      art: this.#art,
