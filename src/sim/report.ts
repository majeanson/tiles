...
export type Summary = {
  ...
  /** Relics carried out, median per run — the meta-economy's income side. */
  readonly medianRelics: number;
  /**
...
export function summarise(policy: string, runs: readonly RunResult[]): Summary {
  ...
  return {
    ...
    medianDepth: median(runs.map((r) => r.reach)),
    deepest: runs.reduce((n, r) => Math.max(n, r.reach), 0),
    ...
    medianRelics: median(runs.map((r) => r.relics)),
    tilesShare: (() => {
...
const COLUMNS: readonly (readonly [string, (s: Summary) => string])[] = [
  ...
  ['relics', (s) => String(Math.round(s.medianRelics))],
