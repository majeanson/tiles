...
const cold = resolveTheme('cold-survey');
...
describe('applyTheme', () => {
  ...
  it('writes every custom property onto the root', () => {
    applyTheme(cold, document.documentElement);
    const style = document.documentElement.style;

    expect(style.getPropertyValue('--bg')).toBe('#0d1214');
    expect(style.getPropertyValue('--danger')).toBe('#c9765a');
    expect(style.getPropertyValue('--tile-yellow')).toBe('#cfc7ae');
    expect(style.getPropertyValue('--font-display')).toContain('Spectral');
    expect(document.documentElement.dataset['theme']).toBe('cold-survey');
  });
  ...
  it('loads one webfont, swaps it, and removes it', () => {
    applyTheme(cold, document.documentElement);
    const links = () => document.querySelectorAll('link#theme-webfont');
    expect(links()).toHaveLength(1);

    // Switching to another direction that wants fonts must not stack a second
    // stylesheet on the first.
    applyTheme(resolveTheme('torchlit'), document.documentElement);
    expect(links()).toHaveLength(1);
    expect(links()[0]?.getAttribute('href')).toContain('Cinzel');

    // And the placeholder asks for none, so a cold start never waits on a
    // third-party host.
